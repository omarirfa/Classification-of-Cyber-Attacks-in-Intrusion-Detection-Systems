...
for step in range(0, numCV):
    ...
    trainL=trainLabels.flatten()
    # trainD=trainData.flatten()
    testL=testLabels.flatten()
    ...
    QDA.fit(trainData, trainL)
    print("testData array: ")
    print(testData.shape)
    print("testLabels array: ")
    print(testLabels.shape)
    prediction["QDA"] = QDA.predict(testData)
    print(accuracy_score(testLabels,prediction["QDA"]))
    # prediction["RandomForest"] = random_forest.predict(X_test)
    print(classification_report(testLabels, prediction['QDA'],labels = [0,1,2,3,5,6,7],target_names = ["Benign","Goldeneye","SSHPatator","FTPPatator","Bot","PortScan","DDos"],output_dict=True)))

    # accuracy_score()
    # prediction["RandomForest"] = random_forest.predict(X_test)
    # print(classification_report(y_test, prediction['RandomForest'],target_names = []))
    # print(random_forest.score(trainData, trainLabels))
